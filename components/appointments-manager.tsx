...
const emptyForm = (): FormState => ({ customerName: "", customerPhone: "", professionalId: "", serviceId: "", date: new Date().toISOString().slice(0, 10), start: "", notes: "" });
...
export function AppointmentsManager({ businessId }: { businessId: string }) {
  ...
  async function loadData() {
    ...
    const [p, s, links, a, b, ap] = await Promise.all([
    ...
    const firstError = p.error || s.error || links.error || a.error || b.error || ap.error;
  ...
  const slots = useMemo(() => {
    ...
    for (const period of periods) {
      for (let start = minutes(period.start_time); start + selectedService.duration_minutes <= minutes(period.end_time); start += 15) {
        ...
        if (!overlapsAppointment && !overlapsBlock) result.push(clock(start));
      }
    }
    return [...new Set(result)].sort();
  }, [appointments, availability, blocks, form.date, selectedProfessional, selectedService]);
  ...
  async function submit(event: FormEvent) {
    ...
    if (conflictError || conflicts?.length) {
      setError(conflictError?.message ?? "Esse horário acabou de ser ocupado. Escolha outro horário."); setSaving(false); return;
    }
    const phone = normalizedPhone(form.customerPhone);
    let clientId: string | null = null;
    if (phone) {
      const { data: existing } = await supabase.from("clients").select("id").eq("business_id", businessId).eq("phone_normalized", phone).maybeSingle();
      if (existing) clientId = existing.id;
      else {
        const { data: created } = await supabase.from("clients").insert({ business_id: businessId, name: form.customerName.trim(), phone: form.customerPhone.trim(), phone_normalized: phone }).select("id").single();
    ...
    else { setOpen(false); setForm(emptyForm()); await loadData(); }
    setSaving(false);
  }
