...
export function SettingsManager({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    async function load() {
      const { data, error: loadError } = await supabase.from("businesses").select("name,segment,phone,city,address,slug,public_description,minimum_notice_hours,booking_window_days,slot_step_minutes,allow_client_cancellation,cancellation_notice_hours,theme,show_prices_publicly,show_professional_specialty").eq("id", businessId).single();
