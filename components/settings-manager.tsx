...
export function SettingsManager({ businessId }: { businessId: string }) {
  ...
  useEffect(() => {
    async function load() {
      const { data, error: loadError } = await supabase.from("businesses").select("name,segment,phone,city,address,slug,public_description,minimum_notice_hours,booking_window_days,slot_step_minutes,allow_client_cancellation,cancellation_notice_hours,theme,show_prices_publicly,show_professional_specialty,whatsapp_confirmation_template").eq("id", businessId).single();
      if (loadError) setError(loadError.message);
      else {
        setSettings(data as Settings);
        document.documentElement.dataset.theme = data.theme === "system" ? "" : data.theme;
      }
    }
    void load();
  }, [businessId, supabase]);
  ...
  async function persist() {
    ...
    if (updateError) setError(updateError.code === "23505" ? "Este endereço público já está sendo usado." : updateError.message);
      ...
      setSettings(payload);
      document.documentElement.dataset.theme = payload.theme === "system" ? "" : payload.theme;
      setSaved(true);
