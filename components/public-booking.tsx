...
import { loadProfessionals, loadServices, ONBOARDING_STORAGE_KEY, ProfessionalRecord, ServiceRecord } from "@/lib/local-data";
...
export function PublicBooking({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    const draft = raw ? JSON.parse(raw) as { business?: Business } : null;
    setBusiness(draft?.business || { name: "Studio Bella", segment: "Beleza e estética", phone: "", city: "", slug });
  ...
  const slots = useMemo(() => {
    ...
    const available = getAvailableSlots({ date, professional: selectedProfessional, service: selectedService, availability: loadAvailability(professionals), blocks: loadBlocks(), appointments: loadAppointments() });
    const minimum = Date.now() + 2 * 60 * 60 * 1000;
    return available.filter((slot) => new Date(`${date}T${slot}:00`).getTime() >= minimum);
  }, [date, professionals, selectedProfessional, selectedService]);
  ...
  function confirm() {
    ...
    const stillAvailable = getAvailableSlots({ date, professional: selectedProfessional, service: selectedService, availability: loadAvailability(professionals), blocks: loadBlocks(), appointments: currentAppointments }).includes(time);
    if (!stillAvailable) {
  ...
  return <main className="public-booking-shell">
    ...
    <section className="public-business"><div className="public-business-avatar">{business?.name?.slice(0,1) || "C"}</div><div><h1>{business?.name || "Carregando agenda..."}</h1><p>{[business?.segment, business?.city].filter(Boolean).join(" · ")}</p></div></section>
    ...
    <section className="public-booking-card card">
      {step === "service" && <><div className="public-heading"><h2>Qual serviço você deseja?</h2><p>Escolha uma opção para continuar.</p></div><div className="choice-list">{services.map((service) => <button className={`choice-card ${serviceId === service.id ? "selected" : ""}`} key={service.id} onClick={() => setServiceId(service.id)}><div><strong>{service.name}</strong><span>{service.duration} min</span></div><b>{service.price}</b></button>)}</div><footer><span /><button className="button button-primary" disabled={!serviceId} onClick={() => go("professional")}>Continuar</button></footer></>}
      {step === "professional" && <><div className="public-heading"><h2>Com quem você deseja agendar?</h2><p>Mostramos apenas profissionais que realizam o serviço escolhido.</p></div><div className="professional-choice-grid">{eligibleProfessionals.map((professional) => <button className={`professional-choice ${professionalId === professional.id ? "selected" : ""}`} key={professional.id} onClick={() => setProfessionalId(professional.id)}><span>{professional.name.slice(0,1)}</span><strong>{professional.name}</strong><small>{professional.specialty}</small></button>)}</div><footer><button className="button button-secondary" onClick={() => go("service")}>Voltar</button><button className="button button-primary" disabled={!professionalId} onClick={() => go("date")}>Continuar</button></footer></>}
      {step === "date" && <><div className="public-heading"><h2>Escolha a data</h2><p>É possível agendar nos próximos 60 dias.</p></div><div className="field"><label>Data do atendimento</label><input className="input" type="date" min={localDate()} max={localDate(60)} value={date} onChange={(event) => { setDate(event.target.value); setTime(""); }} /></div>{date && <div className="date-preview">{formatDate(date)}</div>}<footer><button className="button button-secondary" onClick={() => go("professional")}>Voltar</button><button className="button button-primary" disabled={!date} onClick={() => go("time")}>Ver horários</button></footer></>}
      ...
      {step === "review" && <><div className="public-heading"><h2>Revise seu agendamento</h2><p>Confirme se todos os dados estão corretos.</p></div><div className="booking-review"><div><small>Serviço</small><strong>{selectedService?.name}</strong><span>{selectedService?.price} · {selectedService?.duration} min</span></div><div><small>Profissional</small><strong>{selectedProfessional?.name}</strong><span>{selectedProfessional?.specialty}</span></div><div><small>Data e horário</small><strong>{date && formatDate(date)}</strong><span>{time} às {selectedService && time ? `${String(Math.floor((timeToMinutes(time) + selectedService.duration) / 60)).padStart(2,"0")}:${String((timeToMinutes(time) + selectedService.duration) % 60).padStart(2,"0")}` : ""}</span></div><div><small>Cliente</small><strong>{name}</strong><span>{phone}</span></div></div><footer><button className="button button-secondary" onClick={() => go("customer")}>Voltar</button><button className="button button-primary" onClick={confirm}>Confirmar agendamento</button></footer></>}
