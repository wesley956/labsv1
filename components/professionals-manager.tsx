...
export function ProfessionalsManager({ businessId, initialItems }: { businessId: string; initialItems: Professional[] }) {
  ...
  async function submit() {
    ...
    const query = editingId ? supabase.from("professionals").update(values).eq("id", editingId) : supabase.from("professionals").insert(values);
    const { data, error: requestError } = await query.select("id,name,specialty,phone,active").single();
  ...
  async function toggle(item: Professional) {
    setError("");
    const { data, error: requestError } = await createClient().from("professionals").update({ active: !item.active }).eq("id", item.id).select("id,name,specialty,phone,active").single();
  ...
  return <div className="content">
    ...
    {open && <div style={{position:"fixed",inset:0,zIndex:80,display:"grid",placeItems:"center",padding:20,background:"rgba(6,10,24,.64)"}}><section className="card panel" style={{width:"min(560px,100%)"}}><div className="panel-header"><div><h3>{editingId ? "Editar profissional" : "Novo profissional"}</h3></div><button className="icon-button" onClick={()=>setOpen(false)}>×</button></div><div className="field"><label>Nome completo</label><input className="input" value={form.name} onChange={(e)=>setForm({...form,name:e.target.value})} /></div><div className="field"><label>Especialidade</label><input className="input" value={form.specialty} onChange={(e)=>setForm({...form,specialty:e.target.value})} /></div><div className="field"><label>WhatsApp</label><input className="input" value={form.phone} onChange={(e)=>setForm({...form,phone:e.target.value})} /></div><div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:24}}><button className="button button-secondary" onClick={()=>setOpen(false)}>Cancelar</button><button className="button button-primary" disabled={saving} onClick={submit}>{saving ? "Salvando..." : "Salvar profissional"}</button></div></section></div>}
  </div>;
}
