...
export function AvailabilityManager({ businessId, initialProfessionals, initialAvailability, initialBlocks }: { businessId: string; initialProfessionals: Professional[]; initialAvailability: AvailabilityRow[]; initialBlocks: BlockRow[] }) {
  ...
  async function saveSchedule() {
    ...
    const periods = schedule.flatMap((day) => day.enabled ? day.periods.map((period) => ({
      weekday: day.weekday,
    ...
    if (periods.length > 50) {
      setError("A disponibilidade deve ter no máximo 50 períodos por profissional.");
      return;
    }

    setSaving(true);
    const supabase = createClient();
    const { data, error: saveError } = await supabase.rpc("replace_weekly_availability", {
    ...
    const savedRows = (data ?? []) as AvailabilityRow[];
    setAvailability((current) => [...current.filter((row) => row.professional_id !== selectedProfessionalId), ...savedRows]);
    setSuccess("Horários salvos com segurança.");
  }
  ...
  return <div className="content availability-page">
    ...
    <div className="card availability-toolbar"><div className="field compact-field"><label>Profissional</label><select className="input" value={selectedProfessionalId} onChange={(e) => { setSelectedProfessionalId(e.target.value); setBlockDraft(emptyBlock(e.target.value)); setSuccess(""); }}>{initialProfessionals.map((professional) => <option key={professional.id} value={professional.id}>{professional.name}</option>)}</select></div><div className="availability-person-summary"><div className="avatar">{selectedProfessional?.name.slice(0,1).toUpperCase()}</div><div><strong>{selectedProfessional?.name}</strong><span>{selectedProfessional?.specialty}</span></div></div><button className="button button-secondary" disabled={saving} onClick={copyMondayToWeekdays}>Copiar segunda para dias úteis</button><button className="button button-primary" disabled={saving} onClick={saveSchedule}>{saving ? "Salvando..." : "Salvar horários"}</button></div>
